...
*********************************************************************/
/*
	Modified for ECE6156 Lab by Spandan More
*/
...
// Create the file storing the input keys
void generate_keys()
{
	FILE *outputFile = fopen("Key.txt", "w");

	// Words to write to the file
	const char *keys[] = {
		"Antidisestablish",
		"Disenfranchising",
		"Electrochemistry",
	...
	printf("Succesfully wrote keys to the file.\n");
}

// Create the file storing the input plaintexts
void generate_plaintexts()
{
	FILE *outputFile = fopen("Plaintextin.txt", "w");
	...
	printf("Succesfully wrote plaintexts to the file.\n");
}

// Function to reverse the endianness of a hex string
void reverseEndianness(char *hexString)
{
	int length = strlen(hexString);
	for (int i = 0; i < length / 2; i += 2)
	{
		char temp = hexString[i];
		hexString[i] = hexString[length - 2 - i];
		hexString[length - 2 - i] = temp;

		temp = hexString[i + 1];
		hexString[i + 1] = hexString[length - 1 - i];
		hexString[length - 1 - i] = temp;
	}
}

// Function to create reverse endian ciphertexts for the VHDL simulations
void reverse_ciphertexts()
{
	// Set the base string of all the ciphertext files
	FILE *input, *output;
	char infile_name[20];
	...
	for (int i = 1; i < 6; i++)
	{
		sprintf(infile_name, "Ciphertextout%d.txt", i);
		sprintf(outfile_name, "ct_r_%d.txt", i);

		input = fopen(infile_name, "r");
		output = fopen(outfile_name, "w");

		char hexString[33];
		while (fscanf(input, "%32s", hexString) == 1)
		{
			reverseEndianness(hexString);
			fprintf(output, "%s\n", hexString);
		}
	...
	printf("Succesfully reversed the endianness of the ciphertexts.\n");
}

// Function to create the input ciphertext file for the decrypter
//
// Here, we take the first ciphertext of the first file, second ciphertext of the second file, and so on
void generate_ciphertexts()
{
	FILE *outFile = fopen("Ciphertextin.txt", "w");
	FILE *revOutFile = fopen("ct_r.txt", "w");

	// Take the Xth ciphertext out of CiphertextoutX and write it to an input file
	for (int i = 1; i <= 5; ++i)
	{
		char filename[20];
		sprintf(filename, "Ciphertextout%d.txt", i); // Generate the filename
		...
		// Read the Xth line from CiphertextoutX.txt
		while (1)
		{
			fscanf(inputFile, "%32s", hexString);
			line++;
			if (line == i)
			{
				break;
		...
		// Find the reverse endian representation
		reverseEndianness(hexString);
		fprintf(revOutFile, "%s\n", hexString);
	...
	fclose(outFile);
	fclose(revOutFile);
	printf("Succesfully wrote ciphertexts to the file.\n");
}

// Utility function to print BYTE data
...
void encrypt(WORD *key_schedule)
{
	// Key size is fixed to 128 since all keys used are 16 bytes
	int keysize = 128;

	// Open the files
	FILE *keys = fopen("Key.txt", "r");

	int file_ctr = 1;

	// For all 5 keys
	char key[1024];
	while (fgets(key, sizeof(key), keys) != NULL)
	{
		// Read the key from the file
		size_t len_k = strlen(key) - 1;
		BYTE *k = (BYTE *)malloc(len_k);
		memcpy(k, key, len_k);

		// Set up the key schedule
		aes_key_setup(k, key_schedule, keysize);

		// Open the corresponding ciphertext file
		char filename[20];
		sprintf(filename, "Ciphertextout%d.txt", file_ctr); // Generate the filename

		FILE *ctx = fopen(filename, "w");
		FILE *pts = fopen("Plaintextin.txt", "r");

		// For all plaintexts
		char plaintext[1024];
		while (fgets(plaintext, sizeof(plaintext), pts) != NULL)
		{
			// Convert the plaintext into a byte array
			size_t len_p = strlen(plaintext) - 1;
			BYTE *p = (BYTE *)malloc(len_p);
			memcpy(p, plaintext, len_p);

			// Initialise an ouput array
			BYTE *out = (BYTE *)malloc(len_p);

			// Encrypt the data
			aes_encrypt(p, out, key_schedule, keysize);

			// Write the output to the file
			for (size_t i = 0; i < len_p; ++i)
			{
				fprintf(ctx, "%02x", out[i]);
			}
			fprintf(ctx, "\n");
		}

		// Close the output file and incremenent the file counter
		fclose(ctx);
		fclose(pts);
		file_ctr++;
	}
	...
	printf("Succesfully ecnrypted the plaintexts using the keys.\n");
}
...
unsigned char *hexStringToByteArray(const char *hexString)
{
	...
	int byteLen = len / 2;
	unsigned char *byteArray = (unsigned char *)malloc(byteLen * sizeof(unsigned char));
	...
	for (int i = 0; i < len; i += 2)
	{
		...
		{
			fprintf(stderr, "Invalid hexadecimal character: %c%c\n", hexString[i], hexString[i + 1]);
			free(byteArray);
			return NULL;
		}
...
// Function to decreypt the input ciphertexts into plaintexts using all the given keys
void decrypt(WORD *key_schedule)
{
	// Key size is fixed to 128 since all keys used are 16 bytes
	int keysize = 128;
	...
	while (fgets(key, sizeof(key), keys) != NULL)
	{
		...
		// Open the corresponding ciphertext file
		char filename[20];
		sprintf(filename, "Plaintextout%d.txt", file_ctr); // Generate the filename

		FILE *ptx = fopen(filename, "w");
		FILE *cts = fopen("Ciphertextin.txt", "r");

		// For all ciphertexts
		char ciphertext[1024];
		while (fgets(ciphertext, sizeof(ciphertext), cts) != NULL)
		{
			char *newline = strchr(ciphertext, '\n');
			if (newline != NULL)
				*newline = '\0';

			// Convert the hex string nto a byte array
			BYTE *c = hexStringToByteArray(ciphertext);

			// Initialise an ouput array
			BYTE *out = (BYTE *)malloc(len_k);

			// Encrypt the data
			aes_decrypt(c, out, key_schedule, keysize);

			// Write the output to the file
			for (size_t i = 0; i < len_k; ++i)
			{
				fprintf(ptx, "%02x", out[i]);
			}
			fprintf(ptx, "\n");
		}
	...
	printf("Succesfully decryptes the ciphertexts using the keys.\n");
}

int main()
{
	// Generate the key and the plaintexts
	generate_keys();
	generate_plaintexts();

	// Create a key schedule for the encryption and decryption
	WORD key_schedule[60];

	// Encrypt the plaintexts using the keys
	encrypt(key_schedule);

	// Create the input cipertext file using the previously calculated ciphertexts
	generate_ciphertexts();

	// Decrypt the ciphertexts using all keys
